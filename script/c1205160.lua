...
function s.nfilter(c)
	return c:IsFaceup() and not c:IsCode(CARD_S_CORE)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsFaceup() and not chck:IsCode(CARD_S_CORE) end
	if chk==0 then return Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,CARD_S_CORE),tp,LOCATION_ALL,0,1,nil)
		and Duel.IsExistingTarget(s.nfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil)
