...
function s.spoper(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and Duel.SendtoDeck(Group.FromCards(tc,c),nil,0,REASON_EFFECT) then
		if Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_EXTRA,0, 1,nil,e,tp,code) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.SelectYesNo(aux.Stringid(id,2)) then
