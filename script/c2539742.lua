...
--e2 Effect Code
function s.filter(c)
	return c:IsSpellTrap() and (c:IsFaceup() or c:IsLocation(LOCATION_ONFIELD))
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	and Duel.IsExistingMatchingCard(s.filter,tp,0,LOCATION_GRAVE+LOCATION_SZONE+LOCATION_REMOVED,1,nil) end 
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,Card.IsFaceup,tp,LOCATION_MZONE,0,1,1,nil)
end
function s.operat(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local ct=Duel.GetMatchingGroupCount(s.filter,tp,0,LOCATION_SZONE+LOCATION_GRAVE+LOCATION_REMOVED,nil)
