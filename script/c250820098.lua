...
function s.sftarg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  ...
  for tc in vp:Iter() do
  	if tc:GetOverlayGroup():IsExists(s.sfilter,2,nil) then
    	xyzg:AddCard(tc)
    end
  end
  local b1=Duel.IsExistingTarget(s.sfilter,tp,LOCATION_GRAVE,LOCATION_GRAVE,2,nil)
  local b2=#vp>0 and #xyzg>0
  if chk==0 then return (b1 or b2) end
  if b2 and (not b1 or Duel.SelectYesNo(tp,aux.Stringid(id,2))) then
    if #xyzg==1 then
    	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
      local g=xyzg:GetFirst():GetOverlayGroup():FilterSelect(tp,s.sfilter,2,2,nil)
      Duel.SetTargetCard(g)
    elseif #xyzg>1 then
    	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SELECT)
      local ch=xyzg:Select(tp,1,1,nik)
      Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
      local g=ch:GetOverlayGroup():FilterSelect(tp,s.sfilter,2,2,nil)
      Duel.SetTargetCard(g)
  	end
  else
  	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
    local g=Duel.SelectTarget(tp,s.sfilter,tp,LOCATION_GRAVE,LOCATION_GRAVE,2,2,nil)
   end
...
function s.rcoper(e,tp,eg,ep,ev,re,r,rp)
	...
	if #tc==2 and Duel.SendtoDeck(tc,nil,SEQ_DECKSHUFFLE,REASON_EFFECT)==2 then
