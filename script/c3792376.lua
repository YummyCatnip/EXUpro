...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(s.xyzfil,tp,LOCATION_EXTRA,0,nil)
	if #g>0 and Duel.IsExistingMatchingCard(s.palfil,tp,LOCATION_MZONE,0,1,c) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
		Duel.BreakEffect()
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local sg=g:Select(tp,1,1,nil):GetFirst()
		Duel.XyzSummon(tp,sg)
	end
end
function s.palfil(c)
	return c:IsMonster() and c:IsSetCard(SET_PALEOZOIC) and c:IsFaceup()
end
function s.xyzfil(c)
	return c:IsRank(2) and c:IsXyzSummonable()
end
-- e2 Effect Code
