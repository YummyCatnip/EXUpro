...
function Link.CheckRecursive(c,tp,sg,mg,lc,minc,maxc,f,specialchk,og,emt,filt)
	...
	if c:IsHasEffect(EFFECT_LINK_MAT_RESTRICTION) then
		local eff={c:GetCardEffect(EFFECT_LINK_MAT_RESTRICTION)}
		for _,f in ipairs(eff) do
			if sg:IsExists(Auxiliary.HarmonizingMagFilter,1,c,f,f:GetValue()) then return false end
			local sg2=og:Filter(Auxiliary.HarmonizingMagFilter,nil,f,f:GetValue())
			rg:Merge(sg2)
		end
	end
	local g2=sg:Filter(Card.IsHasEffect,nil,EFFECT_LINK_MAT_RESTRICTION)
	for tc in aux.Next(g2) do
		local eff={tc:GetCardEffect(EFFECT_LINK_MAT_RESTRICTION)}
		for _,f in ipairs(eff) do
...
function Link.CheckRecursive2(c,tp,sg,sg2,secondg,mg,lc,minc,maxc,f,specialchk,og,emt,filt)
	...
	if c:IsHasEffect(EFFECT_LINK_MAT_RESTRICTION) then
		...
		for _,f in ipairs(eff) do
			if sg:IsExists(Auxiliary.HarmonizingMagFilter,1,c,f,f:GetValue()) then return false end
			local sg3=og:Filter(Auxiliary.HarmonizingMagFilter,nil,f,f:GetValue())
			rg:Merge(sg3)
		end
	end
	local g2=sg:Filter(Card.IsHasEffect,nil,EFFECT_LINK_MAT_RESTRICTION)
