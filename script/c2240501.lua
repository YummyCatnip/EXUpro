...
function s.rctarg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:HasNonZeroDefense() end
	if chk==0 then return Duel.IsExistingTarget(Card.HasNonZeroDefense,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,Card.HasNonZeroDefense,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,1,nil)
...
function s.lctarg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
