--Rank-Down-Magic Accursed Force
local s,id,o=GetID()
...
function s.tgfilter(c,e,tp)
	...
	return (#pg<=0 or (#pg==1 and pg:IsContains(c))) and (rk>0 or c:IsStatus(STATUS_NO_LEVEL)) and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,rk-1,pg) 
end
function s.spfilter(c,e,tp,mc,rk,pg)
	if c.rum_limit and not c.rum_limit(mc,e) then return false end
	return mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp) and mc:IsCanBeEffectTarget(e) and c:IsRank(rk) and Duel.GetLocationCountFromEx(tp,tp,mc,c)>0
