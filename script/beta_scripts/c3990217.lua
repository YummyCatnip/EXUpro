...
function s.initial_effect(c)
	...
	c:Quick
	(
		nil,aux.Stringid(id,0),CATEGORY_SPECIAL_SUMMON,nil,nil,nil,
		true,aux.MainPhaseCond(1),s.tributecost(),s.syntg,s.synop,TIMINGS_CHECK_MONSTER|TIMING_MAIN_END
	)
	-- Gain effect for an Xyz Monster used this card as material
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e3:SetCode(EVENT_BE_MATERIAL)
	e3:SetCondition(function(_,_,_,_,_,_,r) return (r&REASON_XYZ)>0 end)
...
function s.tributecost(eff2)
	return function(e,tp,eg,ep,ev,re,r,rp,chk)
		local check
		if eff2 then
			check=Duel.IsExistingMatchingCard(s.tbfilter,tp,LOCATION_DECK,0,1,nil)
		else
			check=aux.CirgonRestrictionCheck(tp) and Duel.IsExistingMatchingCard(s.tbfilter,tp,LOCATION_DECK,0,1,nil)
		end
		if chk==0 then return check end
		if not eff2 then aux.CirgonLock(e:GetHandler(),tp) end
...
function s.syntg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local mg=Duel.GetMatchingGroup(aux.FaceupFilter(Card.IsSetCard,SET_CIRGON),tp,LOCATION_MZONE,0,nil)
...
function s.efop(e,tp,eg,ep,ev,re,r,rp)
	...
	-- Add 1 of its material to the hand or Extra Deck
	rc:SummonedTrigger
	(
		false,false,true,false,aux.Stringid(id,1),CATEGORY_TOHAND+CATEGORY_TOEXTRA+CATEGORY_SPECIAL_SUMMON,
		EFFECT_FLAG_DELAY,true,function(re) return re:GetHandler():IsSummonType(SUMMON_TYPE_XYZ) end,
		s.tributecost(true),s.thtg,s.thop,nil,RESET_EVENT|RESETS_STANDARD
	)
end
...
function s.thfilter(c,e,tp)
	return (c:IsType(TYPE_EXTRA) and c:IsAbleToExtra() or c:IsAbleToHand())
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND|LOCATION_EXTRA,0,1,nil,e,tp,c:GetOriginalCode())
end
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local mg=c:GetOverlayGroup():Filter(s.thfilter,nil,e,tp)
	if chk==0 then return #mg>0 and Duel.GetLocationCount(tp,LOCATION_MZONE,0)>0 end
	Duel.SetOperationInfo(0,CATEGORY_TOEXTRA,mg,1,0,0)
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SendtoHand(tc,tp,REASON_EFFECT)>0 and Duel.GetLocationCount(tp,LOCATION_MZONE,0)>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local sc=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_HAND|LOCATION_EXTRA,0,1,1,nil,e,tp,tc:GetOriginalCode()):GetFirst()
		if sc then
