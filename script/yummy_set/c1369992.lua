...
function s.thfilter(c)
	return c:IsRace(RACE_BEAST) and c:IsAbleToGrave() and c:IsLevelBelow(2) and not c:IsCode(id)
end
	...
	if c:IsRelateToEffect(e) and Duel.SendtoHand(c,nil,REASON_EFFECT)>0 then
		...
		if #g>0 and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
			...
			if Duel.SendtoGrave(tc,REASON_EFFECT)>0 then
				Duel.BreakEffect()
				if tc:IsSetCard(0x147) and Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
