...
function s.atfilter(c)
	return c:IsRace(RACE_BEAST) and c:IsLevel(2) and not c:IsCode(id)
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SendtoHand(c,nil,REASON_EFFECT)>0 then
		...
		local ag=Duel.GetMatchingGroup(s.tafilter,tp,LOCATION_MZONE,0,nil)
		if #g>0 and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
			Duel.BreakEffect()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
			local tc=g:Select(tp,1,1,nil)
			Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(id,3))
			local mg=ag:Select(tp,1,1,nil):GetFirst()
			local ct=mg:GetOverlayCount()
			Duel.Overlay(mg,tc)
			if mg:GetOverlayCount()>ct and Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,aux.Stringid(id,4)) then
