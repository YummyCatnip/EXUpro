...
function s.initial_effect(c)
	...
	e1:SetCost(s.cost)
	e1:SetTarget(s.FusionSummonTarget(nil,s.matfilter,s.extrafil))
	e1:SetOperation(s.FusionSummonOperation(nil,s.matfilter,s.extrafil))
	c:RegisterEffect(e1)
	if not s.global_check then
		...
		temp1:SetTargetRange(LOCATION_MZONE,LOCATION_MZONE)
		...
		temp2:SetTargetRange(LOCATION_ONFIELD,LOCATION_ONFIELD)
...
function s.extramat_temp(e,c)
	return not c:IsLocation(LOCATION_MZONE) and s.extrafilter(c,e:GetHandlerPlayer())
end
...
function s.matfilter(c,e,tp,check_or_run)
	return c:IsSetCard(SET_PUMPKINHEAD) and c:IsAbleToGrave()
end
function s.extrafilter(c,tp)
	return c:IsSetCard(SET_PUMPKINHEAD) and c:IsAbleToGrave() and (not c:IsControler(tp) or not c:IsLocation(LOCATION_MZONE))
end
function s.extrafil(e,tp,mg)
	return Duel.GetMatchingGroup(s.extrafilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil,tp)
end
function s.extramat(e,c)
	return not c:IsLocation(LOCATION_MZONE) and s.matfilter(c)
end
